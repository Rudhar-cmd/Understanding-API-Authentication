...
-   1. No Authentication – Access public data (/random)
-   2. Basic Authentication – Username and password-based access (/all?page=2)
-   3. API Key Authentication – Using an API key as a query parameter (/filter)
-   4. Bearer Token Authentication – Using a token in the request header (/secrets/:id)
...
- - Open Postman and create a new request for each route:
-   1. **GET /noAuth** → No authentication required. URL: http://localhost:3001/noAuth
-   2. **GET /basicAuth** → Set Basic Auth in Postman:
-        - Username: Rudhar_35
-        - Password: XXXX
-   3. **GET /apiKey** → Add query parameter:
-        - Key: apiKey
-        - Value: XXXX
-   4. **GET /bearerToken** → In Headers, add:
-        - Key: Authorization
-        - Value: Bearer XXXX
